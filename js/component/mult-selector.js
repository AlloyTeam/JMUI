...
;(function ($, pro, undefined){
	...
	pro.createWidget('MultSelector', {
		...
		tpl : {
			main: '<div class="jmu-border-1px jmu-multselect"></div>',

			selected : '<% \
						for ( var i = 0, len = Math.min(dat.length, maxRoot); i < len; i++ ) {\
					%>\
						<div class="jmu-multselect-selected jmu-multselect-<%= len %>grid">\
                            <div class="jmu-selected-text-wrap<%= i+1 === len ? "":" jmu-border-1px" %>">\
    				            <span class="jmu-selected-text jmu-no-wrap"><%= dat[i] %></span>\
    				            <i class="jmu-icon-arrow jmu-arrow-down"></i>\
                            </div>\
				        </div>\
					<% } %>',

			option : '<div class="jmu-multselect-panel jmu-border-1px <%= effect %> <%= cla %>">\
							<div class="jmu-multselect-child left" data-panel="left">\
								<ul>\
								<%\
								...
								%>\
									<li class="jmu-border-1px jmu-multselect-child-li <%= isActive %><%= hasThird ? "" : " no-more" %>" data-index="<%=i%>">\
					            		<% if (icon) { %>\
                                            <div class="jmu-multselect-child-li-icon <%= icon %>"></div>\
                                        <% } %>\
                                        <div class="jmu-multselect-child-text jmu-no-wrap"><%= text %></div>\
					            		<i class="jmu-icon-arrow jmu-arrow-right"></i>\
					            	</li>\
				            	<% } %>\
				            	</ul>\
       						</div>\
       						<div class="jmu-multselect-child right" data-panel="right"><ul></ul></div>\
			            	<%\
		...
		_renderRoot: function(){
			...
			if ( selected && selected.length ) {
				this.$roots = this.$el.html($.template(this.tpl.selected, { dat : selected, maxRoot : opts.maxRoot })).find('.jmu-multselect-selected');
			}
        },
        _initPullDown : function () {
			...
			$.each(opts.option, function (k, child) {
				...
				if ( child.tree && child.tree.length ) {
					$roots.eq(k).addClass('jmu-multselect-root-more');
				}
			});
        },
        _bindEvents: function () {
            ...
            .on('tap', '.jmu-multselect-selected', function () {
        		var $elem = $(this);

        		if ( $elem.hasClass('jmu-multselect-root-more') ) {
            ...
            // var that = this;
            this.$body.on('tap', '.jmu-multselect-panel li', function () {
                if ( this.currShowing !== null ) {
                    var $this = $(this);
                    var $parent = $this.parents('.jmu-multselect-child');
            ...
            // hack for android
            if ( $.os.android ) {
                // var $uisearchinput = $(".jmu-search-input")[0];
                var currY, startScrollTop;

                this.$body.on('touchstart', '.jmu-multselect-child[data-bouncefix]', function (e) {
                    e.preventDefault();
                })
                .on('touchstart', '.jmu-multselect-child[data-scrollable]', function (e) {
                ...
                .on('touchmove', '.jmu-multselect-child[data-scrollable]', function (e) {
                    var touch = e.touches[0];
            ...
            if ( typeof index === 'number' && opt ) {
                ...
                if ( index < max ) {
                    ...
                    if ( selected !== options.selected[index] ) {
                        this.$roots.eq(index).find('.jmu-selected-text').text(options.selected[index] = selected);
                    }
                }
                // call _initPullDown
                this._initPullDown();
        ...
        _renderSubPanel : function (root, child) {
            ...
            if ( child.refresh ) {
                ...
                if ( root.$panel ) {
                    // 移除android端兼容事件
                    if ( $.os.android ) root.$panel.find('.jmu-multselect-child').off();
                    // 移除节点
                    root.$panel.remove();
                    delete root.$panel;
                }
        	...
        	if ( !root.$panel ) {
                ...
                // 初始化二级菜单滚动条
                setTimeout( function () {
                    that._initScroll(root.$panel.find('.jmu-multselect-child.left'));
                }, 0 );
        	}
            ...
            if ( this._need2Refresh(child, force) ) {
                ...
                if ( child.hasThirdPanel ) {
                    ...
                    this._renderThirdPanel($panel.find('.jmu-multselect-child.right'), hasThird[1] || []);
                }
                
            }

            // 初始化下拉菜单选择状态
            $panel.find('.jmu-multselect-child.left li').removeClass('active').eq(slSecond).addClass('active');

            if ( slThird !== undefined )
                $panel.find('.jmu-multselect-child.right li').removeClass('active').eq(slThird).addClass('active');

        },
        _renderThirdPanel : function ($parent, children) {
        	$parent.html('<ul>'+$.map(children, function (child, k) {
        		return '<li class="jmu-border-1px jmu-multselect-child-li" data-index="'+ k +'"><div class="jmu-multselect-child-text jmu-no-wrap">'+ child +'</div></li>';
        	}).join('')+'</ul>');
        ...
        _onSelect : function () {
            ...
            if ( this._need2Refresh(child) ) {
                ...
                // refresh root's text
                this.$roots.eq(this.currShowing).find('.jmu-selected-text').text(child.defaultChild || child.defaultParent);
            }
